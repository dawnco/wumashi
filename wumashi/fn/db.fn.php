 ...
 * @author lanlin
 */
function insert_return_sql( $table, $data, $db = NULL ) {
	is_null( $db ) && $db = db();
	$sql = insert_sql( $table, $data, $db );
	run_sql( $sql, $db );
	$data['sql'] = $sql;
	$data['id']  = mysql_insert_id( $db );
	return $data;
}
...
function update( $table, $data, $wheres = array(), $db = NULL ) {
	...
	return run_sql( "UPDATE `{$table}` SET {$update_data} {$update_where}", $db );
}
...
function delete( $table, $wheres, $db = NULL ) {
	...
	return run_sql( "DELETE FROM `$table` {$delete_where}", $db );
}
